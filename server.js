...
app.get('/chat/:roomId', (req, res) => {
  ...
  if (username && username.toLowerCase().includes('rahul')) {
    return res.status(400).send("Username should not contain 'rahul'");
  }

  if (blockedUsers.has(username)) {
    return res.status(403).send("You are blocked from the chat");
  }

  req.session.username = username;  
  res.render('chat', { roomId , username});  
});
